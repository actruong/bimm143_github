---
title: "Lab Class 13: RNA-Seq with DE-Seq2"
...
Let's have a small peek at these two objects: 
```{r}
metadata
```
...
```{r}
# The indicies (i.e. positions) that are "control" 
...
library(ggplot2)
ggplot(meancounts) + 
  aes(control.mean, treated.mean) + geom_point(alpha = 0.3)
```
...
```{r}
ggplot(meancounts) + 
  aes(control.mean, treated.mean) + geom_point(alpha = 0.3) + scale_x_log10() + scale_y_log10()
```
...
```{r}
ggplot(res) + aes(log2FoldChange, -log(padj)) + geom_point(col = mycols) + geom_vline(xintercept = c(-2,+2), col = "red") + geom_hline(yintercept = -log(0.05), col ="red") + xlab("Log2 Fold Change") + ylab("Inverse Change") + labs(title = "Differential Expression of the Effect of Dex On Gene Expression") + theme_minimal(base_size = 14)
```
...
To make sense of our results we need to know what the differential expressed genes are and what biological pathways and processes they are involved in. 
...
> Q. Generate and insert a pathway figure for "Graft-verus-host disease" and "Type I diabetes"? 
