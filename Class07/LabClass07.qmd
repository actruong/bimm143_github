---
title: "Class 07: Machine Learning I"
...
> Q. Plot our clustering results with points colored by cluster and also add the cluster as new points colored blue?
...
k4$tot.withinss
```
> Q. Let's try different nymber of K (centers) from 1 to 30 and see what the best result is? 
...
Note: It's possible to go up to 60 centers, but it is not efficient since the elbow or knee point is at 2 which is the optimal k for clustering. 

**Key-point**: K- means will impose a clustering structure on your data even if it is not there - it will always give you the answer you asked for even if its absurd! 


## Hierarchical Clustering 

The main function for Hierarchical Clustering is called `hclust()` (which does all the work for you) you can't just pass `hclust()`our raw input data. It needs a "distance matrix" like the one returned from the `dist()` function. 
...
One solution is to set the row names is to do it by hand...
...
A better way to do this is to set the row names to the first column with `readcsv()`
...
I prefer the second way because it's more efficient and seems to encompass a more general approach towards it. 

### Spotting major differences and trends 

Is difficult even in this small 17D dataset... 
